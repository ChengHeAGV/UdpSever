    ...
    public partial class DataMonitorForm : Form
    {
        ...
        private void contextMenu_AddDes_Click(object sender, EventArgs e)
        {
            if (doubleBufferListView1.SelectedItems.Count > 0)
            {
                //获取寄存器编号
                int regNum = int.Parse(doubleBufferListView1.SelectedItems[0].SubItems[3].Text);
                DataInfoUpdateForm updateDataInfo = new DataInfoUpdateForm(regNum, doubleBufferListView1.SelectedItems[0].SubItems[6].Text, doubleBufferListView1.SelectedItems[0].SubItems[7].Text);
